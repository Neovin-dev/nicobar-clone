    ...
    <div class="pt-3.5 pl-1.25">
      ...
      <div class="flex items-center justify-between mt-1 mb-1 relative w-full bg-white">
        <div class="flex items-center justify-between w-full mt-1 mb-3 absolute left-0 top-[-0.5] h-2 bg-white  py-2 group-hover:opacity-0 group-hover:invisible transition-opacity duration-300">
          <div class="flex space-x-2 w-full text-[12px] pr-2 bg-white items-center justify-center rounded-sm text-ellipsis overflow flex-nowrap whitespace-nowrap">
...
export default defineComponent({
  ...
  computed: {
    ...
    currentImage(): { id: number, src: string } {
      return this.productData.images[this.loadingImageIndex] || {};
    },
  ...
  methods: {
    async nextImage(step: number) {
      ...
      let newIndex = this.loadingImageIndex + step;
      ...
      const nextImageSrc = this.productData.images[newIndex].src;

      try {
        await loadImage(nextImageSrc);

        this.loadingImageIndex = newIndex;
